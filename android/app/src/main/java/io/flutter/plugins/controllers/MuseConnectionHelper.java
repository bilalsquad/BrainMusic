...
public class MuseConnectionHelper {
    ...
    private static final String CHANNEL_NAME = "com.example.muse_sdk";
    private final MethodChannel methodChannel;

    MethodChannel channel = new MethodChannel(flutterEngine.getDartExecutor().getBinaryMessenger(), CHANNEL_NAME);


    public MuseConnectionHelper(MethodChannel methodChannel) {
...
//            }
    ...
    class DataListener extends MuseDataListener {
        ...
        public void receiveMuseDataPacket(final MuseDataPacket p, final Muse muse) {
            ...
            // Si le type de paquet est EEG, envoyez les données EEG au canal Flutter.
            if (p.packetType() == MuseDataPacketType.EEG) {
                double[] eegData = new double[p.valuesSize()];
                for (int i = 0; i < eegData.length; i++) {
                    eegData[i] = p.getEegChannelValue(i);
                }

                // Convertissez eegData en une liste de doubles (peut-être au format List<Double>).
                List<Double> eegDataList = Arrays.asList(eegData);

                // Envoyez les données EEG au canal Flutter.
                channel.invokeMethod("onEEGDataReceived", eegDataList);
            }
        }
